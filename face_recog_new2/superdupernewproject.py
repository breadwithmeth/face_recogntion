...
def information(iid, con, image, name, left, x, font_size):
    with con.cursor() as cursor:
        ...
        first_name = str(get_values_from_table(cursor, first_name, iid))
        ...
        cv2.putText(image, first_name, (left, int(x + 0 * ofs)), font,font_size, color, font_thickness)
    ...
    while True:
        ...
        for ((top, right, bottom, left), name) in zip(boxes, names):
            ...
            cv2.rectangle(frame, (left, top), (right, bottom),
                (0, 255, 0), 2)
            y = top - 15 if top - 15 > 15 else top + 15
            cv2.putText(frame, name, (left, y), font,
                font_size, color, 1)
...
def webcam_recognize():
    ...
    while True:
        ...
        for ((top, right, bottom, left), name) in zip(boxes, names):
            ...
            #cv2.putText(frame, information(name), (right, x), font, font_size, color, 3)
            font_size = (bottom - top) / 100
            information(name, connection, frame, name, right, y, font_size)
